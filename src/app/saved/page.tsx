...
export default function SavedPage() {
  ...
  const [activeTab, setActiveTab] = useState<'saved' | 'latest'>('saved');
      ...
      <div className="h-full flex flex-col overflow-hidden bg-white">
        ...
        <div className="px-4 pt-12 lg:pt-6 pb-0 flex-shrink-0">
          ...
          <div className="flex gap-6 mt-4">
            ...
            <TabButton active={activeTab === 'latest'} onClick={() => setActiveTab('latest')}>
              Latest
            </TabButton>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-28">
          {activeTab === 'saved' ? (
            ...
            <div className="text-center py-20">
              <div className="text-5xl mb-4">📋</div>
              <p className="font-semibold text-[#0F1729]">Recently saved</p>
              <p className="text-sm text-[#9CA3AF] mt-1">Listings you recently liked</p>
            </div>
          )}
        </div>
      </div>
...
function TabButton({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void }) {
  return (
    <button onClick={onClick} className="relative pb-3">
      <span className={cn('text-base font-bold transition-colors', active ? 'text-[#0F1729]' : 'text-[#9CA3AF]')}>
        {children}
      </span>
      {active && (
        <motion.div
          layoutId="saved-tab"
          className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#0F1729] rounded-full"
        />
      )}
    </button>
  );
